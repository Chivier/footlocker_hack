...
typedef struct Shoes_Node_Tp {
    ...
    void update_price(const float x) { price = x; }

    void update_name(const string b) { name = b; }

    void add_colors(const string col) { colors.push_back(col); }

    bool operator==(const Shoes_Node_Tp other) const { return this->name == other.name; }

    bool operator!=(const Shoes_Node_Tp other) const { return this->name != other.name; }

    bool operator<(const Shoes_Node_Tp other) const { return this->price < other.price; }
...
void ShopList_Deal(const std::string &str) {
    ...
    for (iter = str.begin(); iter < str.end(); ++iter) {
        ...
        if (brackets == 0 && shoe_now != shoe_none)
            Cage[++Cnt] = shoe_now, shoe_now = shoe_none;
...
string Get_Details() {
    system(
        "curl 'https://www.footlocker.com/category/brands/jordan.html' -H 'authority: www.footlocker.com' -H 'pragma: "
        "no-cache' -H 'cache-control: no-cache' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (X11; "
        "Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36' -H 'accept: "
        "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/"
        "signed-exchange;v=b3' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: "
        "en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7' --compressed > rawdata.txt");
